...
import { getInputAndOutputFromUrl } from '../helpers/getInputAndOutputFromUrl'
...
async function fetchTradingPairs(pages = 1, exchangeName = 'pancakeswap_new') {
	...
	for (let i = 0; i < pages; i++) {
		const response = await fetch(
			`https://api.coingecko.com/api/v3/exchanges/${exchangeName}/tickers?page=${
				i + 1
			}`
		)
	...
	if (filteredMapped.length < mapped.length) {
		console.log(
			`Had to remove ${
				mapped.length - filteredMapped.length
			} pairs because they werent contained in the token Lookup. Consider updating the json file`
		)
	}
